          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
            ...
            {status === "success" ? (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                <p className="text-green-700 font-bold text-sm">Message envoyé avec succès !</p>
                <p className="text-green-600 text-xs mt-1">Nous vous répondrons dans les plus brefs délais.</p>
              </div>
            ) : (
              <>
                <button
                  ...
                  className="w-full bg-red-600 text-white font-bold py-3 rounded-lg hover:bg-red-700 transition flex items-center justify-center gap-2 disabled:opacity-60"
                >
                  {status === "loading" ? "Envoi en cours…" : "Envoyer le message"}
                </button>
                {status === "error" && (
                  <p className="text-xs text-red-500 text-center">Une erreur est survenue. Réessayez ou contactez-nous via WhatsApp.</p>
                )}
                <p className="text-xs text-gray-400 text-center">
                  Ou contactez-nous directement sur{" "}
                  <a href={WA_BASE} target="_blank" rel="noreferrer" className="text-[#25D366] font-semibold hover:underline">WhatsApp</a>
                </p>
              </>
            )}
          </form>
